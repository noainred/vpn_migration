...
- 의존성 없음 (Python 3.6+ 표준 라이브러리만) · 폐쇄망/에어갭 동작 · 외부 CDN 없음
...
- **서브넷/IP 보기** — IP를 서브넷별로 묶어 보고, **IP를 클릭하면 그 IP가 어떤
  서버/피어와(어떤 서비스로) 통신하는지** 드릴다운합니다.
- **활동/유휴 시간대** — 피어별 **주중×시간(168칸) 히트맵**과, 네트워크는 활성인데
  그 IP는 무통신인 **유휴 창**(= 안전한 마이그레이션 시간)을 제시합니다.
  타임스탬프(`frame.time`)가 있어야 하며, 메모리는 호스트당 168칸으로 **고정**입니다.
- **토폴로지 전체화면** — `⛶ 토폴로지 전체화면` → 별도 페이지에서 **힘기반 배치**로
  아이콘이 겹치지 않게 펼치고, **휠 확대/축소·드래그 이동·노드 고정**을 지원합니다.
- **분/시/일 주기 저장** — 시스템 대시보드의 *수집 저장 설정*에서 **저장 위치**와
  분·시·일 토글, 보관 개수를 지정하면, 라이브 집계가 그 주기로 `report.json`
  스냅샷으로 저장됩니다(원자적 쓰기, 보관 개수 초과 시 오래된 것 삭제).
- **시스템 대시보드** — 프로세스 **CPU·메모리(RSS)**, 저장 디스크 **여유 용량**,
  **저장 파일 개수/용량**을 3초마다 갱신해 메모리/디스크 누수를 조기 발견합니다.
  (psutil 없이 `resource`+`/proc`+`shutil.disk_usage`로 구현.) **메모리 리셋**
  버튼으로 캡처를 멈추지 않고 누적분만 비울 수 있습니다.
- **다중 서버 통합** — 여러 서버에서 만든 `report.json`을 한 번에 업로드하면
  **통신쌍을 중복 제거하며 통합**한 최종안을 만듭니다. CLI로도:
  ```bash
  tinc-flow-analyzer --merge -f json -o final.json server1.json server2.json server3.json
  ```
